...
def get_domain_age_days(hostname):
    # Skip WHOIS for empty hostnames or IP addresses
    ...
    try:
        ...
        # If creation date is missing, return unknown
        if creation_date is None:
            return None

        # Make date timezone-aware if needed
        ...
        # WHOIS can fail because of rate limits, privacy, or unsupported domains
        return None
def analyze_redirects(url):
    try:
        # Visit the URL with redirects enabled, but do not download the full page body
        ...
        # Network errors should not crash the detector
        return None, None
...
def detect_url(url):
    ...
    # Get protocol such as http or https
    scheme = parsed_url.scheme.lower()

    # Get hostname without username, password, or port
    hostname = parsed_url.hostname

    # Use empty string if hostname is missing
    domain = hostname.lower() if hostname else ""

    # Rule 1: HTTP is weaker than HTTPS
    if scheme == "http":
        risk_score += 20
    ...
    # Rule 3: Long URLs can hide suspicious content
    if len(url) > 75:
        risk_score += 15
    ...
    # Rule 5: @ symbol can hide the real destination
    if "@" in url:
        risk_score += 25
        reasons.append("URL contains @ symbol")

    # Rule 6: Many dots may indicate subdomain abuse
    if domain.count(".") >= 4:
        risk_score += 15
        reasons.append("Domain contains many dots")

    # Rule 7: Many hyphens may indicate fake brand domains
    if domain.count("-") >= 2:
        risk_score += 10
        reasons.append("Domain contains many hyphens")

    # New Rule 8: Domain age checking
    ...
    # If no suspicious reason exists, explain clearly
    if not reasons:
